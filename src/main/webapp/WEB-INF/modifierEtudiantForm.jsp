<%@ page import="com.projetj2e.projetj2e.conn.MyUtils" %>
<%@ page import="java.sql.*" %><%--
  Created by IntelliJ IDEA.
  User: nafkh
  Date: 09/12/2022
  Time: 6:40 PM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<style><%@include file="/inc/css/style.css"%></style>
<div class="modification">
    <div class="information_etudiant">
        <h3>Information Etudiant</h3>
        <span>Numero Inscription : ${Etudiant.numInsc}</span>
        <span>Nom Complet : ${Etudiant.nom} ${Etudiant.prenom} </span>

        <span>Email : ${Etudiant.mail} </span>

        <span>Groupe : ${Etudiant.grp}</span>
        <h3>Modifier Groupe</h3>
        <form method="post" action="${pageContext.request.contextPath}/modifierEtudiant?code=${Etudiant.numInsc}">
            <%
    try {


      Connection conn = MyUtils.getStoredConnection(request);
      Statement stmt = conn.createStatement();
      ResultSet resultSet = stmt.executeQuery("Select distinct idGRP from groupe ");

  %>
  <select name="groupe">
        <% while (resultSet.next()){%>
        <option value="<%= resultSet.getString(1)%>"><%= resultSet.getString(1)%></option>
        <%}%>


        </select>
        <%
            }catch (SQLException e){

            }
        %>
        <input type="submit" value="Modifier Groupe">

        </form>

    </div>


</div>


